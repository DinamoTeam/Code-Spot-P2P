...
function testGeneratePositionBetween(numberOfElements: number): void {
  ...
  const notDeleteRate = numberOfElements / 10; // The bigger, the less deletion
  let numError = 0;
  let totalDeleted = 0;
  let clock = 1;
  ...
  for (let i = 0; i < numberOfElements; i++) {
    ...
    arr.splice(randomIndex, 0, newCRDTId);  // insert new element between randomIndex-1 and randomIndex
