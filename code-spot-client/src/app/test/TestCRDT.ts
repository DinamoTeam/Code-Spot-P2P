...
function testGeneratePositionBetween(numberOfElements: number): void {
  ...
  for (let i = 0; i < numberOfElements; i++) {
    ...
    const numConcurrentlyInsertAtSameSpot = Math.floor(
      Math.random() * MAX_NUM_CONCURRENTLY_INSERT_AT_SAME_SPOT
    );
    const siteId = Math.floor(Math.random() * NUMBER_OF_SITE_ID) + 1;
    const newCRDTIds: CRDTId[] = [];
    const newCRDTId = CRDTId.generatePositionBetween(
      arr[randomIndex - 1],
      arr[randomIndex],
      siteId,
      clock++
    ...
    if (!isCRDTIdArrayIncreasing(arr)) {
      console.error(
        'ERROR: testGeneratePositionBetween() array is not increasing'
      );
      numError++;
    }

    if (arr.length >= 3 && Math.random() < 0.2) {
      // Occasionally delete
      const start = 1 + Math.floor(Math.random() * (arr.length - 2)); // except 0 and arr.length-1
      const numDelete = Math.floor(
        (Math.random() * (arr.length - start)) / notDeleteRate
  ...
  console.log(
    'Tested CRDT.generatePositionBetween() by randomly inserting ' +
      numberOfElements +
      ' elements, and deleting ' +
      totalDeleted +
      ' elements. ' +
      numError +
      ' errors found'
...
// const id1 = new CRDTId([new Identifier(1, 2)], 8);
// const id2 = new CRDTId([new Identifier(35, 3)], 9);

// const idsBetween = CRDTId.generateNPositionsBetween(id1, id2, 10, 4, 10);

// printCRDTIdArray(idsBetween);
